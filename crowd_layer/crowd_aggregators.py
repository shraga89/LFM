...
class CrowdsCategoricalAggregator():
	...
	def __init__(self, model, data_train, answers, batch_size = 16, pi_prior = 1.0):
		...
		self.n_train = int(answers.shape[0])
		self.num_classes = int(np.max(answers) + 1)
		self.num_annotators = int(answers.shape[1])
	...
	def e_step(self):
		...
		for i in range(self.n_train):
			...
			for r in range(self.num_annotators):
				if self.answers[i,r] != -1:
					adjustment_factor *= self.pi[:,int(self.answers[i,r]),r]
	...
	def m_step(self,):
		print("M-step")
		hist = self.model.fit(self.data_train, self.ground_truth_est, epochs=1, batch_size=self.batch_size) 
		print("loss:", hist.history["loss"][-1])
		self.ground_truth_est = self.model.predict(self.data_train)
		...
		for r in range(self.num_annotators):
			...
			for i in range(self.n_train):
				if self.answers[i,r] != -1:
					self.pi[:,int(self.answers[i,r]),r] += np.transpose(self.ground_truth_est[i,:])
