...
def init_identities(shape, partition_info=None, dtype=None):
    out = np.zeros(shape)
...
class CrowdsClassification(Layer):
    ...
    def build(self, input_shape):
        if self.conn_type == "MW":
            ...
            shape = (self.output_dim, self.output_dim, self.num_annotators)
            self.kernel = self.add_weight("CrowdLayer", shape, initializer=init_identities, trainable=True)
...
class MaskedMultiF1score(object):

    def loss(self, y_true, y_pred):
        print('FIX LOSS FUNCTION!')
        vec = tf.nn.softmax_cross_entropy_with_logits(logits=y_pred, labels=y_true, dim=1)
        mask = tf.equal(y_true[:, 0, :], -1)
        zer = tf.zeros_like(vec)
        loss = tf.where(mask, x=zer, y=vec)
...
class CrowdsAggregationCategoricalCrossEntropy(object):
    ...
    def m_step(self):
        # print "M-step"
        self.pi_est = tf.transpose(self.suff_stats / tf.expand_dims(tf.reduce_sum(self.suff_stats, axis=2), 2),
                                   [1, 2, 0])
    ...
    def loss_fc(self, y_true, y_pred):
        ...
        for r in range(self.num_annotators):
            ...
            self.suff_stats_beta_norm[r] += tf.reduce_sum(
                tf.where(tf.equal(y_true[:, r], -1), tf.zeros_like(p), (1.0 - mu)))
